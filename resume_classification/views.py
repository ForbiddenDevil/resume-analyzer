...
def convertpdftodoc(files):
    for file in files:
        ...
        # print(file_ext)
        path = default_storage.save(os.path.join(settings.RC_MEDIA_ROOT, file_name), ContentFile(file.read()))
...
def connectandloaddatafromcsv(host,username,password):
    ...
    read_csv(filepath='./resume_classification/new_resume_data.csv',
...
def datapreprocess(resume_data_test):
    ...
    TextParser_out = TextParser(data=resume_data_test,
    ...
    copy_to_sql(TextParser_out.result, table_name='text_parser_data', if_exists='replace')                
...
def datapreprocess_train(raw_df):
    ...
    TextParser_out = TextParser(data=raw_df,
                                text_column="Resume_str",
                                covert_to_lowercase=True,
                                output_by_word=True,
                                punctuation = "\[.,?\!\]",
    ...
    copy_to_sql(TextParser_out.result, table_name='text_parser_data_train', if_exists='replace')     
...
def classifyresume(tokenize_df):
    ...
    nbt_predict_out = NaiveBayesTextClassifierPredict(object = model_df,
                                                      newdata = tokenize_df,
    ...
    final_predict_df = predict_df.merge(right=max_pred_df , how='inner', on = ["ResID", "prob=max_prob"], lsuffix = 'l', rsuffix = 'r')
    final_predict_df = final_predict_df.assign(drop_columns=True,
...
def evaluatemodel(final_predict_df):
    ...
    ClassificationEvaluator_obj = ClassificationEvaluator(data=final_predict_df,
